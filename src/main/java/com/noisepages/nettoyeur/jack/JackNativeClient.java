...
import org.github.sidec.jackaudiodevice.support.NativeLoader;
...
public abstract class JackNativeClient {
	...
	static {
		NativeLoader.load(JackNativeClient.class);
	}
     ...
     * @param portsOut number of left ports
	...
	public JackNativeClient(String name, int portsIn, int portsOut, boolean isDaemon) throws JackException {
        ...
        if (portsOut < 0 || portsOut > getMaxPorts()) throw new IllegalArgumentException("left ports out of range");
	...
	/**
     * connects all left ports to ports specified by target
     *
	 * @param target regular expression specifying target ports; "" means physical ports, and null means no connection
	 * @return number of connections made
	 */
	...
	/**
     * connects a given range of left ports to ports specified by target
     *
	 * @param port first port to be connected
	 * @param range number of ports to be connected
	 * @param target regular expression specifying target ports; "" means physical ports, and null means no connection
	...
	/**
     * disconnects a range of left ports
     *
	 * @param port first port to be disconnected
	 * @param range number of ports to be disconnected
	 * @return number of ports disconnected
	...
	/**
     * @return the largest admissible number of input/left ports per client
     */
     ...
     * @param out the direct memory access left buffer
     */
	...
	public static void main(String[] args) throws JackException, InterruptedException, IOException {
		JackNativeClient client = new JackNativeClient(UUID.randomUUID().toString(), 1, 2) {
			float phaseIncrementer = (float)(2*Math.PI)*(110f/48000);
			float phase = 0;
			@Override
			protected void process(FloatBuffer[] inBuffers, FloatBuffer[] outBuffers) {
                ...
                for(int i = 0; i < outBuffers.length; i++){
                    FloatBuffer b = outBuffers[i];
                    b.rewind();
                    phase = tmp;
                    for(int j = 0; j < b.capacity(); j++){
                        b.put(((float) Math.cos(phase)));
                        phase +=phaseIncrementer;
                    }

                }
//                System.out.println(outBuffers[0].capacity());
			}
		};
		client.connectInputPorts("system");
		client.connectOutputPorts("system");
        System.out.println(client.getSampleRate());
